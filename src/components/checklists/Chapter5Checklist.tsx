...
export default function Chapter5Checklist({ responses, setResponses, externalCollapsed, setExternalCollapsed }: Props) {
  ...
  const allSectionCodes = ch5Rows.filter((r) => r.isMainSection).map((r) => r.code);
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#111" }}>Chapter 5 — General Building Heights and Areas</div>
          <div style={{ fontSize: 14, color: "#555", fontWeight: 400, margin: "4px 0 0" }}>
            This chapter establishes general limitations on building height, number of stories, and allowable floor area based on occupancy group and construction type.
          </div>
        </div>
      ...
      {ch5Rows.map((row) => {
        ...
        const isHidden = !isMainSection && (() => {
          ...
          let parentCode = "";
          for (const candidate of ch5Rows) {
